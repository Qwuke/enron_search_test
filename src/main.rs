...
fn main() {
    ...
    let email_file_paths =
        get_email_paths_from_dir("/home/qwuke/enron_search_engine/resources/enron/")
...
fn search(input: &str, search_trie: &mut Trie<BString, BTreeMap<BigDecimal, String>>) {
    ...
    let mut matched_prefixes: Vec<(&BString, String, BigDecimal)> = search_trie
        .iter_prefix_mut(&BString::from(sanitized_input.clone()))
        .flat_map(|(word, map)| {
    ...
    if matched_prefixes.is_empty() {
        ...
        matched_prefixes
            ...
            .for_each(|(word, doc, score)| {
                println!(
                    "Email {} matching word {} with score {}",
                    doc,
                    word.as_str(),
                    score
                );
...
fn build_search_trie(
    tf_idf: HashMap<String, HashMap<String, f64>>,
) -> Trie<BString, BTreeMap<BigDecimal, String>> {
    let words_with_ranked_docs =
        tf_idf
            .into_iter()
            .fold(HashMap::new(), |mut acc, (doc, word_score_map)| {
                word_score_map.into_iter().for_each(|(word, score)| {
                    let ordered_score = BigDecimal::from_f64(score).expect("Valid real decimal");
                    acc.entry(word)
                        .and_modify(|map: &mut BTreeMap<BigDecimal, String>| {
                            map.insert(ordered_score.clone(), doc.clone());
                        })
                        .or_insert({
...
fn calc_tf_idf(
    ...
    let document_tf_score = document_word_freq
        .into_iter()
    ...
    let doc_tf_idf = document_tf_score
        .iter()
        .map(|(document, tf_scores)| {
            let tf_idf_scores = tf_scores
                .into_iter()
                .map(|(word, tf_score)| {
                    (
                        word.to_owned(),
                        tf_score * inverse_document_frequency.get(word).unwrap_or(&0.0_f64),
                    )
                })
                .collect::<HashMap<String, f64>>();
...
fn calc_inverse_document_freq(
    document_word_freq: HashMap<String, HashMap<String, u64>>,
) -> HashMap<String, f64> {
    let total_word_freq = document_word_freq
        .values()
        .fold(HashMap::new(), |mut acc, doc_map| {
            doc_map.into_iter().for_each(|(k, v)| {
